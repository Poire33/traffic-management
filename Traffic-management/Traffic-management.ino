...
void setup() {
  ...
  // attachInterrupt(digitalPinToInterrupt(BUTTON_RIGHT), right_beg, FALLING); // Should be a falling edge
  // attachInterrupt(digitalPinToInterrupt(BUTTON_LEFT), left_beg, FALLING); // An empty attachInterrupt doesn't cause any problems but once beg_button(); or other code is added, the board constantly reboots. When the function is set to LOW, nothing happens when the button is pressed. No rebooting.
}
